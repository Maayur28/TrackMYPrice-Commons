...
public class RedisServiceHelper {
    ...
    /* ------------ HASH ops for failed products ------------ */

    // HSET failed:productId productId -> json/value
    public Long putHash(String hashKey, String productId, String value) {
        return redisConnection.sync().hset(hashKey, productId, value) ? 1L : 0L;
    }

    // HGET failed:productId productId -> value or null
    public String getHash(String hashKey, String productId) {
        return redisConnection.sync().hget(hashKey, productId);
    }

    // HDEL failed:productId productId -> 1 if removed, 0 if absent
    public Long deleteHash(String hashKey, String productId) {
        return redisConnection.sync().hdel(hashKey, productId);
    }
    ...
    // Delete ALL failed items in one shot
    public Long deleteAllHash(String hashKey) {
        // One key to drop; use UNLINK to be non-blocking if Redis >= 4
        try {
            return redisConnection.sync().unlink(hashKey);
        } catch (Exception e) {
            // fallback if UNLINK not supported
            return redisConnection.sync().del(hashKey);
        }
    }
